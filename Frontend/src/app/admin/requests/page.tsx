                            ...
                            <div className="bg-slate-50 rounded-xl p-4 mb-6 flex-grow">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="w-full">
                                        {(request.type === RequestType.DriverUpdate || request.type === RequestType.VehicleUpdate) ? (
                                            <div className="space-y-2">
                                                <p className="text-xs font-bold text-slate-500 uppercase mb-2">Пропоновані зміни:</p>
                                                {(() => {
                                                    try {
                                                        const data = JSON.parse(request.comment);
                                                        ...
                                                        const labelMap: Record<string, string> = {
                                                            fullName: 'ПІБ', FullName: 'ПІБ',
                                                            phone: 'Телефон', Phone: 'Телефон',
                                                            licenseNumber: 'Номер посвідчення', LicenseNumber: 'Номер посвідчення',
                                                            status: 'Статус', Status: 'Статус',
                                                            isActive: 'Активність', IsActive: 'Активність',
                                                            model: 'Модель', Model: 'Модель',
                                                            licensePlate: 'Номер авто', LicensePlate: 'Номер авто',
                                                            type: 'Тип транспорту', Type: 'Тип транспорту',
                                                            fuelType: 'Тип палива', FuelType: 'Тип палива',
                                                            fuelConsumption: 'Витрати (л/100км)', FuelConsumption: 'Витрати (л/100км)'
                                                        };
                                                        ...
                                                        const formatValue = (key: string, val: any) => {
                                                            ...
                                                            if (key.toLowerCase() === 'status') {
                                                                const isDriver = request.type === RequestType.DriverUpdate || String(request.type) === '2';
                                                        ...
                                                        // Find changes - always compare and show only what changed
                                                        const changes = Object.entries(data)
                                                            ...
                                                            .filter(([key, value]) => {
                                                                ...
                                                                const formattedCurrent = formatValue(key, currentVal);
                                                                const formattedNew = formatValue(key, value);

                                                                return formattedCurrent !== formattedNew;
                                                            });

                                                        if (changes.length === 0) {
                                                            const isProcessed = request.status !== RequestStatus.Pending && String(request.status) !== '1';
                                                            return <p className="text-sm text-slate-500 italic">
                                                                {isProcessed
                                                                    ? "Змін не виявлено (історія для старих запитів недоступна або дані ідентичні)"
                                                                    : "Змін не виявлено (дані ідентичні поточним)"}
                                                            </p>;
                                                        }
                                                        ...
                                                        return (
                                                            <div className="space-y-3">
                                                                {changes.map(([key, value]) => {
                                                                    ...
                                                                    return (
                                                                        <div key={key} className="flex flex-col border-l-2 border-blue-200 pl-3 py-1">
                                                                            <span className="text-[10px] text-slate-400 font-bold uppercase">{labelMap[key] || key}</span>
